...
import { setUploading } from 'store/dashboardSlice'
...
export async function uploadPack(file) {
  ...
  const id = Date.now()
  store.dispatch(setUploading(previousState.concat({ id, name: file.name })))
  const result = await parsePackGenerator(file)
  store.dispatch(
    setUploading(store.getState().dashboard?.uploading?.filter(pack => id !== pack.id))
  )
  if (result.error) {
    const error =
      {
        noContentXML:
          'Не найден файл content.xml в паке. Возможно, архив поврежден или неправильно создан, например, с абсолютными путями вместо относительных.',
        ...
        'Error: Can\'t find end of central directory : is this a zip file ? If it is, see https://stuk.github.io/jszip/documentation/howto/read_zip.html':
          'Файл не является паком SiGame или архив поврежден.',
    ...
    try {
      ...
      if (await blockedByCors(value)) {
        try {
          ...
          return exit(
            'Не удалось получить пак по указанному адресу, потому что автор сайта настроил политику CORS, блокирующую возможность импортирования паков по URL'
          )
        }
      } else {
