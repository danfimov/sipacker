  ...
  return (
    <div className={styles.uploadingFiles} style={{ display: !props.display && 'none' }}>
      <div className={styles.uploadingScreen}>
          ...
          {stage === 'uploading' && (
            <>
              ...
              <span className={styles.caption}>
                Загрузка файла {uploadingFileIndex} из {filesListLength}
              </span>
            </>
          )}
          {stage === 'uploaded' && (
            <>
              <MdDone />
              <span className={styles.caption}>
                {props.errors.length < filesListLength ? 'Файлы загружены' : 'Операция завершена'}
              </span>
            </>
          )}
        </div>
        ...
        {stage === 'uploaded' && (
          <Button variant='contained' onClick={() => props.setStage('addForm')}>
            ОК
          </Button>
        )}
      </div>
