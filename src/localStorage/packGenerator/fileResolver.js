...
export default class FileResolver {
  ...
  async connect(fileID) {
    ...
    if(fileID.startsWith('@')) {
      ...
      try {
        if(this.resolvedURLs[fileID]) return this.resolvedURLs[fileID]

        let urlFile
        try {
          urlFile = await getUrlFileInfo(fileID)
        } catch(e) {
          // URL is not fetchable (http, cors, unsupported mime, etc.) — store as unknown
          urlFile = {
            type: 'unknown',
            size: null,
            name: fileID.split('/').slice(-1)[0] || 'Без названия'
          }
        }

        const fileURI = await saveFileAsURL(fileID, urlFile, this.packUUID)
        this.resolvedURLs[fileID] = fileURI
        return fileURI
      } catch(e) {
        throw { error: e }
      }
