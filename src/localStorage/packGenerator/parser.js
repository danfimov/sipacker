...
export default async function parse(blob) {
  try {
    ...
    const mapText = array => {
      ...
      return array.elements.map(t => t.elements?.[0]?.text).filter(t => t !== undefined)
    }

    const packageTag = n(content, 'package')
    const { id, name, version, difficulty, restriction, date, publisher, logo, language } =
      packageTag.attributes
    ...
    const rounds = await Promise.all(
      (n(packageTag, 'rounds')?.elements ?? []).map(async round => ({
        name: round.attributes.name,
        themes: await Promise.all(
          (n(round, 'themes')?.elements ?? []).map(async theme => ({
            name: theme.attributes.name,
            questions: await Promise.all(
              (n(theme, 'questions')?.elements ?? []).map(async question => {
                ...
                const questionPriceType =
                  ['bagcat', 'cat'].includes(type) &&
                  (isNaN(typeParam('cost'))
                    ? 'byPlayer'
                    : Number(typeParam('cost')) > 0
                      ? 'fixed'
                      : 'minMax')
                ...
                var realpriceFrom,
                  realpriceStep,
                  realpriceTo,
                  playerSelectingPrice = false
                if (type === 'bagcat' && questionPriceType === 'byPlayer') {
                  playerSelectingPrice = true
                  var [, realpriceFrom, realpriceTo, realpriceStep] =
                    typeParam('cost').match(/^[\\d+;\\d+]\/\\d*$/)
                }
                ...
                return {
                  ...
                  realprice:
                    type === 'cat' || (type === 'bagcat' && questionPriceType === 'fixed')
                      ? Number(typeParam('cost')) || 100
                      : undefined,
                  ...
                  scenario: await Promise.all(
                    (n(question, 'scenario')?.elements ?? []).map(async atom => {
