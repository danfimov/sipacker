...
const packErrors = {
  ...
  noScenario:
    'Добавьте как минимум одно событие в каждом сценарии каждого вопроса в каждом теме каждого раунда',
}
    ...
    elements: [
      {
        type: 'comment',
        comment:
          'This package was generated using SIPacker — free open-source online tool for generating SIGame packs. Feel free to leave feedback on https://git.hloth.dev/hloth/sipacker',
      },
      {
        type: 'element',
        name: 'package',
                        ...
                        elements: [
                          {
                            ...
                            name: 'questions',
                            elements: await Promise.all(
                              theme.questions.map(async question => ({
                                type: 'element',
                                name: 'question',
                                attributes: {
                                ...
                                elements: [
                                  ...questionType(question, theme.name),
                                  {
                                    ...
                                    name: 'scenario',
                                    elements: await Promise.all(
                                      question.scenario.map(async scenarioEvent => ({
                                        type: 'element',
                                        name: 'atom',
                                        attributes: {
                                        ...
                                        elements:
                                          scenarioEvent.type !== 'marker'
                                            ? [
                                              {
                                                type: 'text',
                                                text: {
                                                  text: xmlescape(
                                                    scenarioEvent.data.text ?? ''
                                                  ).replaceAll('\n', '\\n'),
